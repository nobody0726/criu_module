...
cd "$KDIR"
if grep -q '^int criu_cgroup_freeze_threadgroup' include/linux/criu_freezer.h 2>/dev/null &&
	grep -q '^int criu_cgroup_thaw_threadgroup' include/linux/criu_freezer.h 2>/dev/null &&
	grep -q 'EXPORT_SYMBOL_GPL(criu_cgroup_freeze_threadgroup)' kernel/cgroup/cgroup.c &&
	grep -q 'EXPORT_SYMBOL_GPL(criu_cgroup_thaw_threadgroup)' kernel/cgroup/cgroup.c; then

...
grep -Fq '.flags = CLONE_SETTLS' "$carrier_c"
...
grep -Fq 'b1_carrier_unregister_inherited_rseq' "$carrier_c"
grep -Fq '__rseq_offset' "$carrier_c"
grep -Fq 'RSEQ_FLAG_UNREGISTER' "$carrier_c"
grep -Fq 'b1_carrier_unregister_inherited_rseq(&child)' "$carrier_c"

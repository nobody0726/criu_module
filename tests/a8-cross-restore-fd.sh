...
skip() { echo "A8_FD_CROSS_RESTORE: SKIP: ENVIRONMENT ($*)"; exit 77; }
fail() { echo "A8_FD_CROSS_RESTORE: FAIL: $*" >&2; exit 1; }
...
make -C "$ROOT/tests/progs" a8-cross-fd >/dev/null
TMP=$(mktemp -d /tmp/a8-cross-restore.XXXXXX)
loaded=0
root=0
child=0
cleanup() {
	...
	if [ "$rc" -ne 0 ] && [ "$rc" -ne 77 ]; then
		echo "A8_FD_CROSS_RESTORE: FAIL rc=$rc" >&2
...
(cd / && exec env GLIBC_TUNABLES=glibc.pthread.rseq=0 setsid \
	"$ROOT/tests/progs/a8-cross-fd" <"$TMP/stdin" \
...
if ! printf '%s\n' "$root" >"$DEBUG_DIR/target"; then
	echo "target request failed" >&2
	ps waux >&2 || true
	ls -l "/proc/$root" >&2 || true
	grep -E '^(State|Pid|PPid|NSpid):' "/proc/$root/status" >&2 || true
	cat "$DEBUG_DIR/status" >&2 || true
	dmesg | tail -80 >&2 || true
	fail "kernel target rejected fixture root"
fi
if ! printf '%s %s\n' "$root" "$TMP/snapshot.bin" >"$DEBUG_DIR/dump-tree"; then
	echo "dump-tree request failed" >&2
	cat "$DEBUG_DIR/status" >&2 || true
	dmesg | tail -80 >&2 || true
	fail "kernel dump-tree rejected cross-process fixture"
fi
if ! rmmod criu_kernel; then
	echo "rmmod returned non-zero; module state:" >&2
	grep '^criu_kernel ' /proc/modules >&2 || true
	dmesg | tail -40 >&2 || true
	if grep '^criu_kernel ' /proc/modules >/dev/null; then
		fail "module unload failed"
	fi
fi
loaded=0
echo "A8_FD_CROSS_RESTORE: dump complete"
[ -e "$TMP/snapshot.bin" ] || { echo "snapshot missing after rmmod" >&2; ls -la "$TMP" >&2; cat "$DEBUG_DIR/status" >&2 || true; dmesg | tail -80 >&2 || true; }
...
echo "A8_FD_CROSS_RESTORE: conversion complete"
...
echo "A8_FD_CROSS_RESTORE: restore complete"
root=$restored_root
...
printf x >>"$TMP/stdin"
for _ in $(seq 1 200); do
	grep -q '^cross-check=PASS pipe=PASS unix=PASS$' "$TMP/fixture.out" && break
	sleep 0.05
done
grep -q '^cross-check=PASS pipe=PASS unix=PASS$' "$TMP/fixture.out" ||
	fail "cross-process IPC behavior marker missing"
kill -0 "$root"
kill -0 "$child"

#!/bin/sh
# Linux 5.10.29 gate: each type must restore, check its state and respond again.
set -eu
...
run_case() (
	set -eu
	label=$1; fixture=$2; check=$3
	TMP=$(mktemp -d /tmp/a5-cross-restore.XXXXXX)
	pid=0; loaded=0
	cleanup() {
		rc=$?
		# Unload first: a dump failure must not strand a frozen task.
	...
	ulimit -n 4096
	: >"$TMP/stdin"
	(cd / && exec env GLIBC_TUNABLES=glibc.pthread.rseq=0 A5_REG_PATH="$TMP/shared" \
		setsid "$ROOT/tests/progs/$fixture" <"$TMP/stdin" >"$TMP/fixture.out" 2>"$TMP/fixture.err") &
	pid=$!
	...
	(cd / && /bin/busybox timeout "${RESTORE_TIMEOUT:-90}" "$CRIU" restore \
		-D "$TMP/images" --shell-job --restore-detached -v4 --log-file="$TMP/restore.log")
	pid=$reported
	kill -0 "$pid"
	printf x >>"$TMP/stdin"
	for _ in $(seq 1 100); do
		grep -q "^$check=PASS" "$TMP/fixture.out" && break
		sleep 0.05
	done
	grep -q "^$check=PASS" "$TMP/fixture.out"
	printf x >>"$TMP/stdin"
	for _ in $(seq 1 100); do
		grep -q '^alive=1$' "$TMP/fixture.out" && break
		sleep 0.05
	done
	grep -q '^alive=1$' "$TMP/fixture.out"
	kill -0 "$pid"
	echo "A5_$label: PASS (restore, behavior, continued execution)"
)
run_case REGULAR fds-dup regular-check
run_case PIPE fds-pipe pipe-check
run_case UNIX fds-unix-stream unix-check
if dmesg | grep -E 'BUG:|WARNING:|Oops:|scheduling while atomic|possible circular locking'; then
	echo 'A5_CROSS_RESTORE: FAIL (kernel diagnostics)' >&2
	exit 1
fi
